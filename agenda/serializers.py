...
class AgendamentoSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if data_horario.date() in Agendamento.objects.filter(
            email_cliente=email_cliente, confirmado=True
        ).dates("data_horario", "day"):
            raise serializers.ValidationError(
                "Não se pode fazer mais de um agendamento diário."
            )

        if data_horario.date() in Agendamento.objects.filter(confirmado=True).dates(
            "data_horario", "day"
        ):
            raise serializers.ValidationError(
                "Já há um agendamento marcado para este horário."
            )

        for i in Agendamento.objects.all().datetimes("data_horario", "minute"):
            if (
                abs(data_horario - i) < timedelta(weeks=0, days=0, hours=0, minutes=30)
                and confirmado == True
            ):
                raise serializers.ValidationError(
